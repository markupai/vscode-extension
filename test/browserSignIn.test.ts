...
): Promise<import("../src/browserSignIn.js").BrowserSignInResult> {
...
describe("runBrowserSignIn", () => {
  ...
  it("happy path: start → poll-pending → poll-complete → exchange", async () => {
    ...
    const fetchImpl: typeof fetch = vi.fn(async (input: Parameters<typeof fetch>[0]) => {
      ...
      if (url.includes("/oauth/figma/poll")) {
        ...
        if (pollCount === 1) return jsonOk({ status: "pending" });
  ...
  it("opens the authorize_url from /start in the user's browser", async () => {
    ...
    const openExternal = vi.fn<(uri: vscode.Uri) => Thenable<boolean>>(async (uri) => {
      openedUri = uri;
      return true;
    });
    const fetchImpl: typeof fetch = vi.fn(async (input: Parameters<typeof fetch>[0]) => {
      ...
      if (url.includes("/poll")) return jsonOk({ status: "complete", code: "c" });
      if (url.endsWith("/exchange")) return jsonOk({ access_token: "at" });
      return notOk();
    });
  ...
  it("rejects when /start returns a non-OK status", async () => {
    const fetchImpl: typeof fetch = vi.fn(async () => notOk(500));
  ...
  it("times out when /poll keeps returning pending", async () => {
    const fetchImpl: typeof fetch = vi.fn(async (input: Parameters<typeof fetch>[0]) => {
      ...
      if (url.includes("/poll")) return jsonOk({ status: "pending" });
      return notOk();
    });
  ...
  it("rejects when /exchange returns a non-OK status", async () => {
    const fetchImpl: typeof fetch = vi.fn(async (input: Parameters<typeof fetch>[0]) => {
      ...
      if (url.includes("/poll")) return jsonOk({ status: "complete", code: "c" });
      if (url.endsWith("/exchange")) return notOk(400);
      return notOk();
    });
  ...
  it("surfaces a clear error when openExternal fails", async () => {
    ...
    await expect(signIn(fetchImpl, { openExternal: async () => false })).rejects.toThrow(
  ...
  it("tolerates transient poll errors and eventually succeeds", async () => {
    ...
    const fetchImpl: typeof fetch = vi.fn(async (input: Parameters<typeof fetch>[0]) => {
      ...
      if (url.includes("/poll")) {
        pollCount++;
        if (pollCount === 1) return notOk(502); // transient
